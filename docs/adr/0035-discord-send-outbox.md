...
- 現時点では `edit_message` 経路の worker renderer 未実装 (dead letter に落ちる設計)。最初の state transition は settle notice のみ outbox 化した。残りは段階的に移行する (Phase I3 後続作業)。
- `updateAskMessageId` は無条件 UPDATE のため、同一 session に対する outbox 配送と reconciler 再投稿の back-fill 順序が入れ替わると最後勝ち。将来 CAS 版 (`WHERE ask_message_id IS NULL`) への移行が必要。
