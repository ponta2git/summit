...
| 依存合成 | `src/appContext.ts`（`AppContext`） | production は `src/index.ts` の `createAppContext()`、テストは `tests/testing/ports.ts` の `createTestAppContext`。根拠 ADR-0018 |
...
実体は `scripts/dev/reset.ts`。`DATABASE_URL` host が `localhost` / `127.0.0.1` / `::1` / `postgres` 以外なら即 throw。ユーザから「データを消して」「Duplicate ask skipped を解消」等の依頼があれば本コマンドを使うこと。
