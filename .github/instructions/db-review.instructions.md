...
- `DIRECT_URL` は **`drizzle.config.ts` だけ**で参照（`src/**` から参照禁止）。
- migration は `drizzle-kit generate` + `drizzle-kit migrate` のみ。**`drizzle-kit push` 禁止**（生成済み SQL 履歴を正本とするため）。
- スキーマ変更フロー: 編集 → `pnpm db:generate` → `drizzle/` 差分レビュー → `pnpm db:migrate` 適用 → `pnpm db:check` 履歴整合確認。
