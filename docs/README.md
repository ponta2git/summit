...
正本文書同士が矛盾した場合、AI は推測で優先順位を付けない。破壊的操作、secret、単一インスタンス、deploy 禁止窓に関わる矛盾は即時停止する。

仕様に明記されていない場合は次の順で扱う。

1. 締切、週キー、順延、参加条件、状態、custom ID等の業務判断、または`requirements/base.md`の用語変更・新語追加が必要なら実装を止め、明示的な承認を求める。
2. 実装途中で未確定点を示す必要がある場合は`todo(ai): spec clarification needed - <issue>`を置き、PRの要確認事項と対応させる。
3. 正本文書の範囲内で可逆かつ小さい技術判断だけは進めてよい。置いた仮定はPR本文へ記録する。
