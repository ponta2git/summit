...
Phase I3 レビュー (docs/reviews/2026-04-21/final-report.md H3) で、state transition と Discord 送信の間に存在する crash / transient failure window が残っていると指摘された。現状は以下の形で同期配送している:
...
- 送信レイテンシが worker tick 周期 (現 `CRON_OUTBOX_WORKER_SCHEDULE` = 10s) 分だけ追加される。ユーザ体感の send は ≤10 秒遅延。後続の `OUTBOX_WORKER_BATCH_LIMIT` 次第でキュー滞留時はより遅延する。
...
- 現時点では `edit_message` 経路の worker renderer 未実装 (dead letter に落ちる設計)。`send_message` renderer も ask initial post / postpone message 等の embed+component payload を未サポート。settle 通知を outbox 化した初期実装は FR second-opinion H1 で「settle 通知は ≤10s 遅延・postpone vote は直接送信」による UX 順序逆転が指摘され、settle 通知は**直接送信に戻した**。outbox infrastructure は本 PR で完成させ、次フェーズで renderer coverage (ask/postpone/reminder/decided) を揃えた上で全送信経路を段階移行する。現状はインフラ整備フェーズであり、`cancelAsking` の `outbox` 引数は将来移行用に残る。
...
- docs/reviews/2026-04-21/final-report.md — H3 finding
