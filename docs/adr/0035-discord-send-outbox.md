...
- 現時点では `edit_message` 経路の worker renderer 未実装 (dead letter に落ちる設計)。`send_message` renderer も ask initial post / postpone message 等の embed+component payload を未サポート。settle 通知を outbox 化した初期実装は FR second-opinion H1 で「settle 通知は worker tick 周期ぶん遅延し postpone vote が先に届く」UX 順序逆転が指摘され、settle 通知は**直接送信に戻した**。outbox infrastructure は本 PR で完成させ、次フェーズで renderer coverage (ask/postpone/reminder/decided) を揃えた上で全送信経路を段階移行する。現状はインフラ整備フェーズであり、`cancelAsking` の `outbox` 引数は将来移行用に残る。
