...
| Interaction / command 設計 | [0004](./0004-discord-interaction-architecture.md), [0016](./0016-customid-codec-hmac-rejected.md), [0023](./0023-cancel-week-command-flow.md), [0032](./0032-status-command.md) |
| test で DB / 時計を差し替える方法 | [0018](./0018-port-wiring-and-factory-injection.md) |
| エラーを throw する / しない、neverthrow の範囲 | [0015](./0015-error-core-apperror-neverthrow.md), [0021](./0021-neverthrow-scope-reaffirmed.md) |
| 新しい feature をどこに置く / 境界は | [0025](./0025-features-directory-migration.md), [0026](./0026-boundary-rationalization.md), [0027](./0027-ui-colocation-and-shared-boundary.md), [0028](./0028-viewmodels-as-feature-assets.md), [0037](./0037-feature-locality-over-cross-cutting-extraction.md) |
...
| Discord 送信の整合性（crash / transient failure） | [0033](./0033-startup-invariant-reconciler.md), [0035](./0035-discord-send-outbox.md), [0036](./0036-reconnect-replay.md) |
| 代替案は既に却下されているか | [0017](./0017-rejected-architecture-alternatives.md), [0021](./0021-neverthrow-scope-reaffirmed.md) |
...
| Tag | ADR |
|---|---|
| db | 0001, 0003, 0008, 0009, 0012, 0019, 0023, 0024, 0026, 0031, 0033, 0035 |
...
| docs | 0006, 0010, 0013, 0014, 0017, 0020, 0022, 0025, 0026, 0027, 0028, 0029, 0030, 0037 |
| testing | 0018 |
| dev-tools | 0029 |
...
- **`src/discord/shared/`** — 真 cross-cutting のみ（dispatcher / guards / customId / channels / viewModelInputs / discordErrors）。feature 固有は置かない（ADR-0026, ADR-0027, ADR-0028）。
- **`src/time/`** — JST / ISO week / 締切計算の一元化（ADR-0002）。
- **`src/db/`** — schema / repositories / client / `ports.ts` / `rows.ts`（ADR-0003, ADR-0018, ADR-0026）。
- **`src/scheduler/`** — cron 登録・tickRunner・reconciler・outboxWorker（ADR-0033, ADR-0035, ADR-0036）。
- **infra SSoT** — `src/env.ts` / `src/config.ts` / `src/slot.ts` / `src/logger.ts` / `src/appContext.ts`（ADR-0013, ADR-0018, ADR-0022, ADR-0030）。
...
scheduler/  ──► features/*.settle, features/*.send, reconciler, outboxWorker
index.ts    ──► scheduler/, dispatcher, appContext, members/, healthcheck/
```

feature 相互依存は避ける。共通化が必要なら `discord/shared/` へ抽出（ADR-0025, ADR-0026, ADR-0027）。pure な型 + builder の feature 間 import のみ許容（ADR-0028）。ただし昇格は責務 cross-cutting かつ feature locality を損なわない場合に限る（ADR-0037）。副作用を伴う feature 間 import は shared 抽出ではなく scheduler / orchestrator 側で解消（ADR-0037）。
