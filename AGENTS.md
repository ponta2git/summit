...
| DB schema / migration | `.github/instructions/db-review.instructions.md` + `drizzle.config.ts` |
...
4. **DIRECT_URL と DATABASE_URL の混同**: `DATABASE_URL` はアプリ（pooled）、`DIRECT_URL` は migration（unpooled）。アプリ env に `DIRECT_URL` 含めない。
