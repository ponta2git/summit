...
- `DIRECT_URL` はアプリ env に含めない（momo-db の `drizzle.config.ts` 専用）。
...
- neverthrow は Interaction handler pipeline、cross-feature orchestration、scheduler application operation の境界で使う（ADR-0053）。DB / Discord / outbox 等の複数 I/O を順序合成する exported flow は `ResultAsync<..., AppError>` を優先する。
...
- cron / timer の runtime callback は `Promise<void>` と `runTickSafely` で隔離する。scheduler operation は `ResultAsync` を返し、runtime adapter で unwrap して `AppError.code` と文脈識別子を構造化ログに含める。
